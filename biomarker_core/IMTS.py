import numpy as np
def compute_imts(t, m, tumour_fraction: float) -> float:
    ...
    if not np.isfinite(tumour_fraction) or not 0 <= tumour_fraction <= 1:
        raise ValueError("tumour_fraction must lie in [0, 1]")
    denominator = float(np.sum(t*t) + np.sum(m*m))
    tumour_sum = float(t.sum())
    if denominator == 0 or tumour_sum == 0:
        return 0.0
    return float(np.sum(t*m)/denominator * (m.sum()/tumour_sum) * tumour_fraction)
